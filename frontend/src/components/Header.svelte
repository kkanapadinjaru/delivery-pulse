    ...
    <div class="header-actions">
      {#if currentUser}
        <span class="user-info">
          {displayName}
          {#if isManager}<span class="role-badge">Manager</span>{/if}
        </span>
      {/if}
      {#if isManager}
      <button class="icon-btn" on:click={openSettings} title="Settings" aria-label="Open settings">
      ...
      {/if}
      <button class="icon-btn" on:click={openHelp} title="Metrics Help" aria-label="Open metrics help">?</button>
      {#if currentUser}
      <button class="logout-btn" on:click={handleLogout} title="Sign out">Sign out</button>
      {/if}
    </div>
  </div>
</header>
...
<style>
  header {
    ...
    color: white;
    padding: 0.6rem 2rem;
    box-shadow: 0 2px 8px rgba(32, 100, 115, 0.25);
  }
